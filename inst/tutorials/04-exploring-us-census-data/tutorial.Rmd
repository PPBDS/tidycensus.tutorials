...
We are requesting the data in wide format, which will spread the estimate and margin of error information across the columns.

### Exercise 3

Copy paste your previous code and assign it to an object called `ga_wide` using the assignment operator `<-`. Print `ga_wide` to verify the results. 
...
Run the function `ggplot()` on `ga_wide`, setting the `aes()` argument to `x = midincE`. Hit "Run Code."
...
Note that this will return a plain graph since we have not mapped the data point into the graph yet.
...
<!-- GK: Using quadratic (?) regression should be more appropriate. Two choices: Add another question right after which changes the argument to method or add that observation as a knowledge drop. -->
...
Assign your code to an object called `metros`. Hit "Run Code."
...
Add title, subtitle, and x and y-axis labels, caption to the plot using `labs()`. 
...
metros |>
  ...
  ggplot(aes(y = reorder(NAME, estimate), x = estimate)) + 
  geom_col() + 
  labs(title = "Percent of commuters taking public transportation to work in the 20 largest US metro areas",
       subtitle = "New York City has the highest percentage of commuters taking public transportation to work",
       x = "Percent of commuters taking public transportation to work",
       y = "Metro area",
       caption = "Source: 2019 ACS 1-year estimates")
```
