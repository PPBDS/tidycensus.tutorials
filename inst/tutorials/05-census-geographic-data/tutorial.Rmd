...
Type `states()` and assign it to an object called `st`. Hit "Run Code."
...
Type st in the first line. In the second line, run `plot()` below with the `geometry` column as the argument. Remember we extract columns using the `$` operator. 
...
Create a variable below named `nm_counties` and assign it to the function `counties()` with the argument `"NM"`. below that, run `plot()` with the `geometry` column extracted from the variable.
...
Run the function `tracts()` with the arguments `"NM"` and `"Los Alamos"` and assign it to the variable `la_tracts`. Then below that, run `plot()` with the `geometry` column extracted from the variable. 
...
Run the function `area_water()` with the argument `"NM"` and `"Los Alamos"` and assign it to the variable `la_water`. Then below that, run `plot()` with the `geometry` column extracted from the variable. 
...
This should produce a bunch of what seems like random points, which represent all the landmarks in DC. We've created dots with landmarks, but what if we want to study something like roads? Lines would be more useful.
