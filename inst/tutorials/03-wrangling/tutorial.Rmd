...
Copy the code above. Get function `map_dfr()` with the first argument as `years`. Inside the `map_dfr()` function, set the second argument to `~{ get_acs() }`. It is fine to have an error.  
...
```{r handling-margins-of-error-with-8, exercise = TRUE}

```

```{r handling-margins-of-error-with-8-hint-1, eval = FALSE}
salt_lake |> ...
```

```{r handling-margins-of-error-with-8-test, include = FALSE}
salt_lake |> mutate()
```
...
```{r handling-margins-of-error-with-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button> 

```{r handling-margins-of-error-with-9-hint-1, eval = FALSE}
salt_lake |>
...
```{r handling-margins-of-error-with-9-test, include = FALSE}
salt_lake |>
  mutate(sex = case_when(
...
```{r handling-margins-of-error-with-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button> 

```{r handling-margins-of-error-with-10-hint-1, eval = FALSE}
salt_lake |>
  mutate(sex = case_when(
    str_sub(variable, start = -2) < "26" ~ "Male",
    TRUE ~ "Female"
...
```{r handling-margins-of-error-with-10-test, include = FALSE}
salt_lake |>
  mutate(sex = case_when(
    str_sub(variable, start = -2) < "26" ~ "Male",
    TRUE ~ "Female"
