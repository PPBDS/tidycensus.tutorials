...
Run the function `ggplot()` on `ga_wide`, setting the `aes()` argument to `x = medincE`. Note that this will return a plain graph.
...
Run the function `ggplot()` on `metros`, setting the `aes()` argument to `x = NAME`, and `y = estimate`. Hit "Run Code."
