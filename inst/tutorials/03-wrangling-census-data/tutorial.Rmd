...
```{r groupwise-census-data-analysis-3, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r groupwise-census-data-analysis-3-hint-1, eval = FALSE}
az_race_percent |>
  summarize(... = median(...)) 
```

```{r groupwise-census-data-analysis-3-test, include = FALSE}
az_race_percent |>
...
```{r groupwise-census-data-analysis-5-test, include = FALSE}
mn_hh_income |>
  filter(variable != "B19001_001") 
...
```{r comparing-acs-estimates-over-t-13-hint-1, eval = FALSE}
... |> 
  arrange(NAME, ..., ...)
```
...
```{r calculating-groupwise-margins-3, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button> 

```{r calculating-groupwise-margins-3-hint-1, eval = FALSE}
salt_lake |>
  mutate(sex = case_when(
    str_sub(variable, start = -2) < "26" ~ "Male",
    TRUE ~ "Female"
...
```{r calculating-groupwise-margins-3-test, include = FALSE}
salt_lake |>
  mutate(sex = case_when(
    str_sub(variable, start = -2) < "26" ~ "Male",
    TRUE ~ "Female"
