...
<!-- DK: Use the modern pipe |> throughout. DONE -->
...
<!-- DK: Fix formatting. Space between ### and Topic title. One blank row around titles, and code chunk and text. DONE -->

<!-- DK Everything after the initial section needs a thorough review and clean up. -->

<!-- DK: Delete test cases which download data. DONE-->
...
```{r group-wise-census-data-analysis-1, exercise = TRUE}

```

```{r group-wise-census-data-analysis-1-hint-1, eval = FALSE}
az_race_percent |>
  group_by(...)
```

```{r group-wise-census-data-analysis-1-test, include = FALSE}
az_race_percent |>
  group_by(NAME)
```
...
```{r group-wise-census-data-analysis-2, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r group-wise-census-data-analysis-2-hint-1, eval = FALSE}
az_race_percent |>
  group_by(NAME) |> 
  filter(... == max(...))
```

```{r group-wise-census-data-analysis-2-test, include = FALSE}
az_race_percent |> 
...
Pipe the dataset `az_race_percent` to the function `group_by()` passing in `variabe` as an argument.

```{r group-wise-census-data-analysis-3, exercise = TRUE}

```

```{r group-wise-census-data-analysis-3-hint-1, eval = FALSE}
az_race_percent |>
  group_by(...) 
```

```{r group-wise-census-data-analysis-3-test, include = FALSE}
az_race_percent |>
  group_by(variable) 
```
...
```{r group-wise-census-data-analysis-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r group-wise-census-data-analysis-4-hint-1, eval = FALSE}
az_race_percent |>
  group_by(variable)|>
  summarize(... = median(...)) 
```

```{r group-wise-census-data-analysis-4-test, include = FALSE}
az_race_percent |>
  group_by(variable) |>
...
```{r group-wise-census-data-analysis-6, exercise = TRUE}

```

```{r group-wise-census-data-analysis-6-hint-1, eval = FALSE}
 mn_hh_income |>
  filter(... != "B19001_001") 
  
```

```{r group-wise-census-data-analysis-6-test, include = FALSE}
mn_hh_income |>
  filter(variable != "B19001_001") 
...
Pipe the previous code into the function `mutate()` setting `incgroup` to `case_when()` as an argument. Within `case_when()` pass in `variable < "B19001_008" ~ "below35k"`, 
    `variable < "B19001_013" ~ "bw35kand75k"`, and
    `TRUE ~ "above75k"` as arguments. Set the whole thing equal to `mn_hh_income_recode`.
    
```{r group-wise-census-data-analysis-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r group-wise-census-data-analysis-7-hint-1, eval = FALSE}
 mn_hh_income_recode <- mn_hh_income |>
  filter(... != "B19001_001")  |>
  mutate(incgroup = case_when(
    variable < "B19001_008" ~ ..., 
...
```{r group-wise-census-data-analysis-7-test, include = FALSE}
mn_hh_income_recode <- mn_hh_income |>
  filter(variable != "B19001_001")  |>
  mutate(incgroup = case_when(
    variable < "B19001_008" ~ "below35k", 
...
Pipe the dataset `mn_hh_income_recode` into the function `group_by`. Pass in argument `GEOID` and `incgroup`

```{r group-wise-census-data-analysis-8, exercise = TRUE}

```

```{r group-wise-census-data-analysis-8-hint-1, eval = FALSE}
 mn_hh_income_recode |>
  ...(GEOID, ...)
  
```

```{r group-wise-census-data-analysis8-test, include = FALSE}
mn_hh_income_recode |>
...
Pipe the previous code into the function `summarize()` setting `estimate` to the `sum()` of `estimate` as an argument. 

```{r group-wise-census-data-analysis-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r group-wise-census-data-analysis-9-hint-1, eval = FALSE}
mn_hh_income_recode |>
  group_by(GEOID, incgroup) |>
  summarize(... = sum(...))
...
```{r group-wise-census-data-analysis-9-test, include = FALSE}
mn_hh_income_recode |>
  group_by(GEOID, incgroup) |>
