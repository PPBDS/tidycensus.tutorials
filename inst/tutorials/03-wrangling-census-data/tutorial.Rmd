...
<!-- DK: Don't use group_by unless you have to. With most dplyr commands, you can just drop the group_by and add .by to the next dplyr step in the pipe. But this is not guaranteed to always work, so you need to test. And don't worry about keeping around a group_by, but then highlight that you did so. -->
...
In the *Tidyverse*, split-apply-combine is implemented with the `group_by()` function. The `az_race_percent` tibble is an example of a data set suitable for group-wise data analysis.  Pipe `az_race_percent` to the function `group_by()` passing in `NAME` as an argument:
...
We can deploy group-wise data analysis to identify the largest racial or ethnic group in each county in Arizona.

### Exercise 2

Pipe the previous code into the function `filter()` setting `percent` to `max()` `percent` as an argument. `group_by()` is an older dplyr function that is seldom used now. Instead its use it superseded by `.by`. Delete the `group_by()` and instead pass in another argument to `filter()`, `.by` = `NAME`.
...
We can read the analytic pipeline as “Create a new dataset, `largest_group`, by using the `az_race_dataset` THEN grouping the dataset by the NAME column THEN filtering for rows that are equal to the maximum value of percent for each group."

### Exercise 3

Pipe the dataset `az_race_percent` to the function `group_by()` passing in `variable` as an argument.
...
Pipe the previous code into the function `summarize` setting `median_pct` to the `median()` of `percent` as an argument. The result of this operation tells us the median county percentage of each racial and ethnic group for the state of Arizona. 
...
`group_by()` is commonly paired with the `summarize()` function in data analysis pipelines. `summarize()` generates a new, condensed dataset that by default returns a column for the grouping variable(s) and columns representing the results of  functions applied to those groups.
