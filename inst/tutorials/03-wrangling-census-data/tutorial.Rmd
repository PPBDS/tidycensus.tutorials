...
percent_college_by_year <- college_by_year |>
  summarize(numerator = sum(estimate),
            denominator = first(summary_est), .by = c(NAME, year)) |>
...
#   year = 2020
# )
# write_rds(salt_lake, "data/salt_lake.rds")

salt_lake <- read_rds("data/salt_lake.rds")
...
<!-- DK: Discuss group_by() DONE -->

<!-- DK: Don't use group_by unless you have to. With most dplyr commands, you can just drop the group_by and add .by to the next dplyr step in the pipe. But this is not guaranteed to always work, so you need to test. And don't worry about keeping around a group_by, but then highlight that you did so. DONE -->

<!-- DK Everything after the initial section needs a thorough review and clean up. -->


<!-- A better workflow is to have the student run the download code. Then, tell them that we have assigned the result of that download to an object. Then, next question, she prints out the object. This is better than having them assign to an object themselves since that task does not result in any output which we can then talk about. DONE -->
...
The split-apply-combine model of data analysis, is a powerful framework for analyzing demographic data.
...
A function is then applied to each group in turn. Finally, the results of the function applied to each group are combined back into a single dataset, allowing the analyst to compare the results by group.

### Exercise 1

Behind the scenes we have assigned the previous code to the variable `az_race_percent`. Print out `az_race_percent` to ensure it works

```{r groupwise-census-data-analysis-1, exercise = TRUE}

```


```{r groupwise-census-data-analysis-1-hint-1, eval = FALSE}
az_race_percent
```

```{r groupwise-census-data-analysis-1-test, include = FALSE}
az_race_percent
```

### Exercise 2

Pipe tibble `az_race_percent` into the function `filter()` setting `percent` to `max()` `percent` and `.by` = `NAME` as arguments.

```{r groupwise-census-data-analysis-2, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r groupwise-census-data-analysis-2-hint-1, eval = FALSE}
az_race_percent |>
  filter(... == max(...), ... = NAME)
```

```{r groupwise-census-data-analysis-2-test, include = FALSE}
az_race_percent |> 
  filter(percent == max(percent), .by = NAME)
```

We can read the analytic pipeline grouping the dataset by the NAME column THEN filtering for rows that are equal to the maximum value of percent for each group."

### Exercise 4

Pipe `az_race_percent` into the function `summarize` setting `median_pct` to the `median()` of `percent` as an argument. Pass in another argument, setting `.by` equal to `variable` The result of this operation tells us the median county percentage of each racial and ethnic group for the state of Arizona. 

```{r groupwise-census-data-analysis-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r groupwise-census-data-analysis-4-hint-1, eval = FALSE}
az_race_percent |>
  summarize(... = median(...)) 
```

```{r groupwise-census-data-analysis-4-test, include = FALSE}
az_race_percent |>
  summarize(median_pct = median(percent), .by = variable)
```


### Exercise 5

Commonly, analysts will need to calculate new custom groups to address specific analytic questions. As such, an analyst might take the following steps: 1) recode the ACS variables into wider income bands; 2) group the data by the wider income bands; 3) calculate grouped sums to generate new estimates.Pass the function `get_acs()` passing in these arguments: geography = `"county"`, `table = "B19001"`,`state = "MN"`, and `year = 2016`.
  
```{r groupwise-census-data-analysis-5, exercise = TRUE}

```

```{r groupwise-census-data-analysis-5-hint-1, eval = FALSE}
get_acs(
  geography = ...,
  ... = "B19001",
  state = "MN",
...
Pipe the dataset `mn_hh_income` to the function `filter()` passing in `variable` is not equal to `"B19001_001"` as an argument

```{r groupwise-census-data-analysis-6, exercise = TRUE}

```

```{r groupwise-census-data-analysis-6-hint-1, eval = FALSE}
 mn_hh_income |>
  filter(... != "B19001_001") 
  
```

```{r groupwise-census-data-analysis-6-test, include = FALSE}
mn_hh_income |>
  filter(variable != "B19001_001") 
...
Pipe the previous code into the function `mutate()` setting `incgroup` to `case_when()` as an argument. Within `case_when()` pass in `variable < "B19001_008" ~ "below35k"`,  `variable < "B19001_013" ~ "bw35kand75k"`, and `TRUE ~ "above75k"` as arguments. 
    
```{r groupwise-census-data-analysis-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r groupwise-census-data-analysis-7-hint-1, eval = FALSE}
mn_hh_income |>
  filter(... != "B19001_001")  |>
  mutate(incgroup = case_when(
    variable < "B19001_008" ~ ..., 
    variable < ... ~ "bw35kand75k", 
    ... ~ "above75k"
  ))
```

```{r groupwise-census-data-analysis-7-test, include = FALSE}
mn_hh_income |>
  filter(variable != "B19001_001")  |>
  mutate(incgroup = case_when(
    variable < "B19001_008" ~ "below35k", 
...
```
 ...
 Within `case-when()` the first condition is evaluated, telling the function to assign the value of below35k to all rows with a variable value that comes before `"B19001_008"` - which in this case will be `B19001_002` (income less than $10,000) through `B19001_007` (income between $30,000 and $34,999). The second condition is then evaluated for all those rows not accounted for by the first condition. This means that case_when() knows not to assign `"bw35kand75k"` to the income group of $10,000 and below even though its variable comes before B19001_013. The final condition in `case_when()` can be set to `TRUE` which in this scenario translates as “all other values.”
 

### Exercise 8

We have assigned the result of the previous pipe to a new object, `mn_hh_income_recode`. Type `mn_hh_income_recode` in the box and hit "Run Code." 

```{r groupwise-census-data-analysis-8, exercise = TRUE}

```

```{r groupwise-census-data-analysis-8-hint-1, eval = FALSE}
mn_hh_income_recode
```

```{r groupwise-census-data-analysis-8-test, include = FALSE}
mn_hh_income_recode
```
...
Pipe `mn_hh_income_recode` to `summarize()` setting `estimate` to the `sum()` of `estimate` as an argument, with `.by` equal to `c(GEOID, incgroup)`. 

```{r groupwise-census-data-analysis-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r groupwise-census-data-analysis-9-hint-1, eval = FALSE}
mn_hh_income_recode |>
  summarize(... = sum(estimate), .by = c(GEOID, ...))
```

```{r groupwise-census-data-analysis-9-test, include = FALSE}
mn_hh_income_recode |>
  summarize(estimate = sum(estimate), .by = c(GEOID, incgroup))
```
...
### Exercise 5
 ...
 Pipe the function `get_acs()`. Inside the function, set `geography` = `"county"`, `variables` = `"DP02_0068P"`, `state` = `"CO"`, `survey` = `"acs1"` and `year` = `2019`.  
...
Copy the code above. Get function `map_dfr()` with the first argument as `years`. Inside the `map_dfr()` function, set the second argument to `~{ getacs() }`. It is fine to have an error.  
...
We have assigned the previous code to the variable `college_by_year`. Print `college_by_year`
...
Copy the code above. Continue using the pipe `|>` to the function `summarize()`. For the first argument of `summarize()`, set `numerator` equal to `sum(estimate)`. For the second argument of `summarize()`, set `denominator` to `first(summary_est)`, and for the third set `.by` equal to `NAME` and `year`. 
...
college_by_year |>
  summarize(numerator = sum(estimate),
            denominator = first(summary_est), .by = c(..., ...)) |>
  mutate(... = ... * (numerator / ...)) 
...
The chapter outlines a `group_by() |> summarize()` workflow, however the use of `group_by` is largely outdated, and superseded by the argument `.by` for functionality. We use it for calculating the percentage of the population age 25 and up with a 4-year college degree, then uses the `pivot_wider()` function from the **tidyr** package to spread the years across the columns for tabular data display. If you want to know more information about the `pivot_wider()` function click [here](https://tidyr.tidyverse.org/reference/pivot_wider.html)
...
Get the function `get_acs()`. Within the function, set `geography` = `"tract"`, `variables` = `vars`, state = `"Utah"`, `county` = `"Salt Lake"`, and `year` = `2020`. 
...
We have assigned the code above to the variable `salt_lake`. Type `salt_lake` and run code to make sure it works.

```{r handling-margins-of-error-with-5-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r handling-margins-of-error-with-5-hint, eval = FALSE}
salt_lake
```
...
```{r calculating-groupwise-margins-1, exercise = TRUE}

```

```{r calculating-groupwise-margins-1-hint-1, eval = FALSE}
salt_lake |> ...
```

```{r calculating-groupwise-margins-1-test, include = FALSE}
salt_lake |> mutate()
```
...
```{r calculating-groupwise-margins-2, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button> 

```{r calculating-groupwise-margins-2-hint-1, eval = FALSE}
salt_lake |>
...
```{r calculating-groupwise-margins-2-test, include = FALSE}
salt_lake |>
  mutate(sex = case_when(
...
Pipe the previous code into the function `summarize()` passing in the argument setting `sum_est` to the `sum()` of `estimate`. Add another argument setting `sum_moe` to `moe_sum()` with argument `moe` and `estimate` and on setting `.by` equal to both `GEOID` and `sex`.

```{r calculating-groupwise-margins-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button> 

```{r calculating-groupwise-margins-4-hint-1, eval = FALSE}
salt_lake |>
  mutate(sex = case_when(
    str_sub(variable, start = -2) < "26" ~ "Male",
    TRUE ~ "Female"
  ...
  summarize(... = ...(estimate), 
            ... = moe_sum(..., estimate), .by = c(..., ...))
```

```{r calculating-groupwise-margins-4-test, include = FALSE}
salt_lake |>
  mutate(sex = case_when(
    str_sub(variable, start = -2) < "26" ~ "Male",
    TRUE ~ "Female"
  ...
  summarize(sum_est = sum(estimate), 
            sum_moe = moe_sum(moe, estimate), .by = c(GEOID, sex))
```

We employed a familiar  summarize(.by = ...) method to aggregate our data by Census tract and sex. Notably, the call to summarize() includes a call to tidycensus’s moe_sum() function, which will generate a new column that represents the margin of error around the derived sum.
