...
use `get_acts()` on the dataset oglala_lakota_age. Within the function, set `geography` = `"county"`, `state` = `"SD"`, `county` = `"Oglala Lakota"`, `table` =  `"B01001"` and `year` = `2020`. 
...
Use the same code as before, changing the `year` = `2010`. You should get an error. 
...
```{r comparing-acs-over-t-7-hint, eval = FALSE}
college_vars <- c("...",
                  "B15002_016",
                  "B15002_017",
                  "...",
...
```{r comparing-acs-over-t-8-hint, eval = FALSE}
years <- ....
names(..) <- years
```
...
```{r comparing-acs-over-t-9-hint, eval = FALSE}
...
```{r comparing-acs-over-t-10-hint, eval = FALSE}
years <- 2010:2019
names(years) <- years
...
```{r comparing-acs-over-t-11-hint, eval = FALSE}
... %>% 
  arrange(NAME, ..., ...)
```
...
```{r comparing-acs-over-t-13-hint, eval = FALSE}
... <- college_by_year %>%
  group_by(..., year) %>%
