...
Let's learn how to use tidyverse tools to gain insight on US Census data. We (mostly) follow the approach of [*Analyzing US Census Data: Methods, Maps, and Models in R*](https://walker-data.com/census-r/index.html) by Kyle Walker. You will learn key functions inside the tidyverse and tidycensus package including get_acs(), filter(), and arrange(). You will also learn how 
...
use `get_acts()` on the data set oglala_lakota_age. Within the function, set `geography` to `"county"`, `state` to `"SD"`, `county` to `"Oglala Lakota"`, `table` to  `"B01001"` and `year` to `2020`. 

```{r first-section-use-se-15, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-section-use-se-15-hint, eval = FALSE}
oglala_lakota_age <- ...(
  geography = "...",
  state = "...",
  ... = "Oglala Lakota",
...
###
Now lets look at the same data from 2010. 

### Exercise 16
Use the same code as before, changing the `year` to `2010`. You should get an error. 

```{r first-section-use-se-16, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-section-use-se-16-hint, eval = FALSE}
oglala_lakota_age <- get_acs(
  geography = "county",
  state = "SD",
  county = "Oglala Lakota",
...
```{r first-section-use-se-17, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-section-use-se-17-hint, eval = FALSE}
oglala_lakota_age <- get_acs(
  geography = "county",
  state = "SD",
  county = "...",
...
### Exercise 18
 ...
 Set the dataset `co_college` to the function `get_acs()`. Inside the function, set `geography` to `"county"`, `variables` to `"DP02_0068P"`, `state` to `"CO"`, `survey` to `"acs1"` and `year` to `2019`.  
 

```{r first-section-use-se-18, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-section-use-se-18-hint, eval = FALSE}
... <- get_acs(
  ... = "county",
  variables = "...",
  ... = "CO",
...
```{r first-section-use-se-19, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-section-use-se-19-hint, eval = FALSE}
co_college19 <- get_acs(
  geography = "county",
  variables = "DP02_0068P",
  state = "CO",
...
Set the dataset `ak_income_compare` to the function `get_acs()`. Inside the function, set `geography` to `"county"`, `variables to `income15 = "CP03_2015_062"` and `income20 = "CP03_2020_062"`, `state` to `"AK"`, and `year` to `2020`.

```{r first-section-use-se-20, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-section-use-se-20-hint, eval = FALSE}
... <- get_acs(
  ... = "county",
  variables = c(
    income15 = "...",
